...
BATCH_SIZE_VALUES=(64 128 192 256 512 1024)
NUM_REP=5
MAX_STEPS=20
...
for MODEL_CONFIG in "${MODEL_CONFIGS[@]}"; do
    read -r MODEL_TYPE MODEL_PATH <<< "$MODEL_CONFIG"

    for USE_LIGER in "${USE_LIGER_VALUES[@]}"; do
        for BATCH_SIZE in "${BATCH_SIZE_VALUES[@]}"; do
            echo "Running with model=$MODEL_TYPE, use_liger=$USE_LIGER and batch_size=$BATCH_SIZE"
